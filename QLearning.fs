...
let net = NeuralNetwork.init [width*height;middleLayer;width]
...
let rec playPreferences (board:Board) (mightBeFull:bool) (player:Player) (pref: int list):(Board * int) option = 
    ...
    if mightBeFull && (Array.forall (fun x -> x = Empty) (Array.concat board)) then
        None
    else 
        match pref with
        | x::xs -> 
            ...
            if legalMove then
                ...
                playPreferences board false player xs 
        | [] -> None
...
let playATurn (robot:Robot) (board:Board) =
    ...
    let orderedPreferences = 
        ...
        |> orderPrefernces   
        |> List.map snd
    let afterPlay = playPreferences board true robot.Color orderedPreferences
    match afterPlay with
    | Some (newBoard,playedCol) -> 
        (newBoard, {
            robot with Memory = {BoardState=board; PlayedMove=playedCol}::robot.Memory
        })
...
let lazyLast vals = 
    (LazyList.rev >> LazyList.head) vals

let listLast vals  =
    (List.rev >> List.head ) vals


let sizeOfOutput (net:NeuralNetwork):int =
    net
    |> lazyLast 
    |> LazyList.length
...
let rec playAGame (p1: Robot) (p2: Robot) (board: Board) : Player option =
    ...
    if p1.Color = p2.Color then
        failwith "Players must be differenc colors"
        ...
        if winner newBoard p1.Color then
            Some p1.Color
        else
            playAGame p2 p1WithMemory newBoard
